  ...
  <div class="homepage">
    <div ref="container">
              ...
              >
                ...
                <template #num>
                  <van-stepper
                    ...
                    class="qtysums"
                    @change="changeqty"
                    @plus="addcss2(item._id, index)"
                    @minus="reducss2(item._id, index)"
                    @overlimit="overmits"
                  />
                </template>
              </van-card>
      ...
      <van-empty description="购物车里空空如也~~" v-else>
        <van-button round type="danger" size="small" plain to="/home"
          >去购买</van-button
        >
      </van-empty>
      <!-- 批量删除 -->
...
export default {
  ...
  computed: {
    totalPrices() {
      return (
        this.cartlists
          ...
          .reduce((prev, item) => prev + item.price * item.qty, 0) * 100
      );
    },
    cartlists() {
    ...
    checkAll: {
      get() {
        return this.cartlists.every((item) =>
  ...
  methods: {
    ...
    delShpos(id) {
      ...
      this.$Dialog
        .confirm({ title: "删除", message: "确认删除当前商品?" })
        .then(() => {
          //    console.log("确认删除");
          this.$request
            .delete("/carts/deltes/" + id)
            .then((resolve) => {
              const { data } = resolve;
              if (data.code === 200) {
