    ...
    <div class="homepage">
        ...
        <ul>
           ...
           <van-goods-action style="z-index:10">
            ...
            <van-goods-action-icon icon="cart-o" text="购物车" :badge="cartlists2" @click="toMycart" />
            <van-goods-action-icon icon="shop-o" text="店铺" badge="12" />
            <van-goods-action-button type="warning" text="加入购物车" @click="addToCart2" />
            <van-goods-action-button type="danger" text="立即购买" @click="rightBuy" />
            </van-goods-action>
            <!-- 立即购买 -->
    </div>
</template>
...
export default {
    ...
    methods:{
      ...
      getCureData(){
        ...
        this.$request.get("/goods/list",{params:{size:20,page:1}}).then((resolve)=>{
      ...
      goto(id){
        ...
        addToCart2(){
           const current = this.cartlists.find((item)=>item._id === this.shoplists._id)
           console.log("current",current);
           if(current){
                this.$store.commit("modifyqty",{id:this.shoplists._id,qty:current.qty+1})
            ...
            // 
             this.$request.post("/carts/addshops",{
            ...
            }).then((resolve)=>{console.log("添加结果",resolve)
            const {data} = resolve;
            if(data.code===200){
              this.$Toast.success('添加购物车成功')
            }else{
        ...
        rightBuy(){
            this.$Dialog.confirm({title: '提交购买',message: '即将前往购物车....',}).then(() => {
                this.$router.push({path:"/mycart"})
